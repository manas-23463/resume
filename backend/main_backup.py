...
async def process_resumes(
    ...
    """Process uploaded resumes and categorize them"""
    try:
        print(f"Received {len(resumes)} resume files for processing")
        for i, resume in enumerate(resumes):
            print(f"  File {i+1}: {resume.filename}")
        ...
        print(f"Using job description: {final_job_description[:100]}...")
        ...
        for resume_file in resumes:
            ...
            try:
                ...
                print(f"Read {len(content)} bytes from {resume_file.filename}")
                ...
                print(f"Extracted {len(text)} characters of text from {resume_file.filename}")
                
                # Extract basic information with strict validation
                ...
                print(f"Extracted info - Name: '{name}', Email: '{email}', Phone: '{phone}'")
                ...
                if user_id and FIREBASE_AVAILABLE:
                    try:
                        ...
                        firebase_service.store_uploaded_file(user_id, file_data)
                        print(f"Stored uploaded file info for {resume_file.filename}")
                    except Exception as file_error:
                        print(f"Error storing uploaded file info: {file_error}")
                
                # Analyze resume with GPT-4o mini
                print(f"Starting GPT analysis for {resume_file.filename}")
                analysis = analyze_resume_with_gpt(text, final_job_description)
                score = analysis["score"]
                category = categorize_resume(score)
                
                print(f"GPT analysis complete - Score: {score}, Category: {category}")
                
                # Simplified resume data structure
                resume_data = {
                    "id": f"resume_{len(results['selected']) + len(results['rejected']) + len(results['considered'])}",
                    "name": name,
                    "email": email,
                    "phone": phone,
                    "s3_url": s3_url,
                    ...
                    "score": score,
                    "category": category,
                    "content": text,  # Store the actual resume content
                    "explanation": analysis.get("explanation", "No explanation provided"),
                    "strengths": analysis.get("strengths", []),
                    "weaknesses": analysis.get("weaknesses", [])
                }
                
                results[category].append(resume_data)
                print(f"Successfully processed resume: {resume_file.filename} -> {category}")
                ...
                # Add a fallback resume entry even if processing fails
                fallback_resume = {
                    "id": f"resume_{len(results['selected']) + len(results['rejected']) + len(results['considered'])}",
                    "name": "Unknown",
                    "email": "",
                ...
                results["rejected"].append(fallback_resume)
                print(f"Added fallback entry for {resume_file.filename}")
            finally:
